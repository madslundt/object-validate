...
## install
`npm install --save simple-schema-validation`
...
```js
import Schema from "simple-schema-validation"
...
Nesting with parent having `required = false` is also possible.
In this case the properties in parent will only be validated if address is not null or undefined.
...
If set to false and object is null or undefined children are not validated.
