...
describe("Messages", () => {
  ...
  describe(".type()", () => {
    test("should return the correct message", () => {
      class A {}
      expect(Messages.type("a", {}, "string")).toBe(
  ...
  describe(".length()", () => {
    test("should return the correct message", () => {
      ...
      expect(Messages.length("a", {}, { min: 2 })).toBe(
        "a must have a minimum length of 2."
      );
      expect(Messages.length("a", {}, { max: 2 })).toBe(
        "a must have a maximum length of 2."
      );
      expect(Messages.length("a", {}, { min: 2, max: 4 })).toBe(
        "a must have a length between 2 and 4."
      );
    });
  });
  ...
  describe(".enum()", () => {
    test("should return the correct message", () => {
      expect(Messages.enum("a", {}, ["b", "c"])).toBe(
